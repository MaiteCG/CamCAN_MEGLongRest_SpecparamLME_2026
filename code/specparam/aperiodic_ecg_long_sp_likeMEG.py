...
aperiodic_mode = 'fixed' # 'fixed' or 'knee' (only for specparam)
# NOTE: This above should be knee, since it is the ECG, but we ran it with fixed mode to compare the goodness of fit with the knee mode.
...
# Set up log file
logfile = os.path.join(logdir, f'aperiodic_ecg_long_{package}_{proc}_{frangestr}{sinterp}.log')
...
def main():
    ...
    for id in subjects:
        ...
        for phase in phases_to_process:
            ...
            psdfilename = f'sub-{id}_task-{task}_proc-{proc}{channelselection}likemeg_desc-dur{cropdata}sepo{epochdur}s{powabbr}_psd.hdf5'
            ...
            if os.path.isfile(savefile) and not overwrite:
                msg = f'Subject {id} in phase {phase}, ECG aperiodic parameters already computed, skipping.'
                print(msg)
                logger.info(msg)
                continue
            ...
            spectra, freqs = psd.get_data(picks=['ecg'], return_freqs=True)
