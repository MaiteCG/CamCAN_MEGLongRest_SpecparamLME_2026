"""
Linear Mixed-Effects (LME) on Spectral Parameters from rest MEG data using Age factors 
and 6 Control Covariates.

This script executes longitudinal linear mixed-effects regression analyses on 
subject resting-state brain parameters while concurrently adjusting for six 
methodological, anatomical, and behavioral confounds: biological sex, total 
intracranial volume (TIV), head translation coordinates (X, Y, Z positions), 
and average head motion displacement. Multiple comparisons across all MEG 
channels and frequency bands are corrected non-parametrically using an empirical 
maximum T-statistic distribution.

Purpose:
- To verify that the longitudinal impacts of baseline age (Age0), time elapsed 
  between waves (deltaAge), and their interaction are robust to major physical 
  and biological confounds.

Processing Steps:
1. Loads the primary subject resting-state brain parameters dataset.
2. Imports and merges the comprehensive covariates table ('meglong_covariates.tsv').
3. Standardizes and z-scores continuous nuisance covariates.
4. Formulates the expanded multi-covariate LME regression matrix.
5. Fits the full regression models across all channel-by-band brain metrics.
6. Executes 10,000 shuffling permutations to build an extreme-value null 
   distribution of the maximum absolute T-statistic.
7. Derives family-wise error rate corrected p-values for primary age effects.
...
def main():
    ...
    # ---- Read covariates ----
    covariatesfile = os.path.join(maindir, 'meglong_covariates.tsv')
