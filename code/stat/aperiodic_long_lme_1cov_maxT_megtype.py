# aperiodic_long_lme_1cov_maxT.py
'''
Filename: aperiodic_long_lme_1cov_maxT.py
Author: Maité Crespo García
Description: Runs the LME model analyses for each parameter of interest (BAND POWER periodic parameters or aperiodic exponent) INCLUDING ONE COVARIATE (band power or aperiodic exponent from empty room, or from ECG), and corrects for multiple comparisons across parameters using the maximum T statistics method with permutations. The variables of interest are read from a tsv file created in a previous step (sens/aperiodic_long_sp_createtsv.py) containing all the variables (aperiodic/periodic parameters) for all subjects, phases and sensor types (gradiometers, magnetometers). Similarly, the covariate variable is read from a tsv file created in sens/aperiodic_long_emptyroom_sp_createtsv_ecg.py.      

Date: 20-01-2026 (created, modified from aperiodic_long_lme_allcov_maxT.py)
Version: 1.0
'''
#
#module load conda
