"""
Aperiodic Parameter Extraction for ECG Channel using SpecParam.

This script extracts spectral parameters by fitting the SpecParam (FOOOF) model 
to the Power Spectral Density (PSD) of the ECG channel. By using the same frequency 
range and model constraints as the MEG analysis, it allows for a direct comparison 
between heart and brain age effects on the aperiodic exponent.

Processing Steps:
1. Loads ECG PSD data generated by 'psd_ecg.py'.
2. Removes the first 30 seconds of the PSD to exclude initial settling time, and 
    selects only the "good" epochs based on a separate bad epochs file. Then averages 
    the PSD across the selected epochs.
3. Interpolates the 23.4 Hz noise (Golan's noise) if specified. For the main analysis,
    this interpolation is performed, but it was skipped for a control analysis.
4. Configures the SpectralGroupModel with specific constraints (e.g., peak 
   width limits, maximum peaks).
6. Exports a .tsv file containing the aperiodic parameters (offset, 
   exponent) and periodic peak data for every MEG sensor.

Author: Maité Crespo García
Affiliation: MRC Cognition and Brain Sciences Unit, Cambridge, UK
Date: 19-Feb-2026 (last modified)
"""
