"""
Power Spectral Density (PSD) Computation for the ECG Channel.

This script extracts and computes the power spectrum of the Electrocardiogram 
(ECG) channel. It uses the same parameters (2s epochs, Welch/Multitaper) 
as the resting-state MEG analysis to allow for direct comparison.

Processing Steps:
1. Loads the raw MEG data (which contains the ECG channel).
2. Applies frequency and notch filters, then resamples, then crops and segments 
    the data into 2-second fixed-length epochs.
3. Computes the PSD specifically for 'ecg' type channels.
4. Saves the psd in HDF5 format.
