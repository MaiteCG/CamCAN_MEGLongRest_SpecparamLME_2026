...
component = 'total' #'aperiodic'  # 'peak' #
withknee = '' if component == 'total' else withknee
...
# Set up log file
logfile = os.path.join(logdir, f'aperiodic_long_{component}_{package}_{proc}_{frangestr}{sinterp}.log')
    ...
    for id in subjects:
        ...
        for phase in phases_to_process:
            ...
            for megtype in megtypes:
                ...
                savefilename = f'sub-{id}_task-{task}_proc-{proc}_desc-{psddesc}{megtype}{fitting_param}{withknee}_{package}{sinterp}_{component}.npy'
                savefile = os.path.join(save_megdir, savefilename)

                # --- Check if the output file already exists ---
                if os.path.isfile(savefile) and not overwrite:
                    msg = f'Subject {id} in phase {phase}, MEG type {megtype}: component {component} of {task} already computed, skipping.'
                    print(msg)
                    logger.info(msg)
                    continue
                ...
                if component !='total':
                    ...
                    for ind in range(len(fg)):
                        cfm = fg.get_model(ind, regenerate=True)
                        data = cfm.get_data(component, space)   
                        spectra.append(data) 
                ...
                data = {'spectra': spectra, 'freqs': freqs, 'channels': ch_names, 'component': component, 'space': space}
                #print(spectra.shape)
                ...
                t1 = time.time()   
                deltat = t1 - t0
                msg = f'Subject {id} s {component} component computed in {deltat:.2f} seconds.'
                print(msg)
                logger.info(msg)
