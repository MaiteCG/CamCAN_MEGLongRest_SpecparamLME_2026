...
ignorenoisepeaks = False #True #  whether to ignore band peaks that fell within the interpolated noise interval (21.9-23.9 Hz)
...
savedir = os.path.join(loaddir, f'lme_maxT_{fitting_param}_{bandsdiv}{sinterp}{signorenoisepeaks}_{proc}_emptyroom_{num_rand}rand')
...
# Set up logging
logfile = os.path.join(logdir, f'aperiodic_long_{proc}_{fitting_param}_{bandsdiv}{sinterp}_lme_maxT.log')
...
def main():
    ...
    datafile_allmeans = f'aperiodic_stier_{proc}_emptyroom_{fitting_param}_{bandsdiv}{sinterp}{signorenoisepeaks}_allvars_means.tsv'
            ...
            for megtype in megtypes:            
                for var in varsoi:
                    ...
                    datafile = os.path.join(
                        loaddir, f'aperiodic_stier_{proc}_emptyroom_{fitting_param}_{megtype}{var}_{bandsdiv}{sinterp}.tsv'
                    )         
    ...
    if irand is not None:
        ...
        if not os.path.exists(statorifile) or overwrite or True:
            ...
            tori, pori, cori = lme_age(df, goodvars, effects_of_interest, return_coefficients=True)
