"""
Aperiodic and Periodic Parameter Extraction using SpecParam (FOOOF).

This script extracts spectral parameters by fitting a model to the Power Spectral 
Density (PSD) of MEG data to separate the 1/f aperiodic signal (offset and exponent) 
from periodic peaks (centre frequency, power, band width). It can be applied to 
the PSDs of rest MEG (main analysis) or control MEG datasets processed with different 
ICA treatments.

Processing Steps:
1. Loads the PSD data generated by 'psd_rest.py'.
...
5. Fits the model to a defined frequency range (default: 2-40 Hz).
