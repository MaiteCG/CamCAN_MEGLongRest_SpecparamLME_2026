"""
Linear Mixed-Effects (LME) Modeling with Permutation-Based Max-T Correction.

This script executes longitudinal linear mixed-effects (LME) models on the aggregated 
spectral and aperiodic parameters. It tests for the effects of baseline age (Age0), 
longitudinal age changes (deltaAge), and their interaction. To address the multiple 
comparisons problem across parameters, the script implements a non-parametric permutation 
test using the maximum T-statistic distribution to control the family-wise error rate.

Purpose:
- To test for the effects of baseline age (Age0), longitudinal time changes 
  (deltaAge), and their interaction (Age0:deltaAge) on brain parameters.
- To protect against type 1 errors (false positives) across dozens of neural 
  metrics without using overly conservative traditional corrections like 
  Bonferroni.

Processing Steps:
1. Configures the parameter directories based on custom data-cleaning pipelines.
2. Formulates frequency band boundaries based on user choices (such as 
   splitting the beta band into 2 or 3 distinct components).
3. Reads the master data metrics table containing channel and participant means.
4. Fits an LME model for each variable using pymer4.
5. Shuffles data over thousands of random iterations to build an empirical 
   null distribution of the maximum T-statistic.
6. Compares the real T-statistics against this null distribution to derive 
   corrected p-values.
