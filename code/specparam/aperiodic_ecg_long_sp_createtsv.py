'''
Filename: aperiodic_ecg_long_sp_createtsv.py
Author: Maité Crespo García
Description: Creates a tsv file with all the variables (aperiodic/periodic parameters) estimated by FOOOF on the ECG channel, FOR ALL  SUBJECTS and PHASES. It saves it in a statistics directory. The input data are the tsv files created with the script sens/aperiodic_ecg_long_sp.py, which contain the aperiodic/periodic parameters for each subject, phase. The peak parameters in these input files are not yet classified into a classic frequency bands (theta, alpha, beta, etc.), so this script also classifies them into bands and extracts the relevant parameters per band (peak frequency, band power, etc.) to create the final tsv file for statistics.         
Date: 08-Jan-2026 (created, modified from aperiodic_long_sp_createtsv.py)
Version: 1.0
'''
